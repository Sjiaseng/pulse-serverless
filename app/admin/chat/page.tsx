...
function LiveChatPageContent() {
  ...
  useEffect(() => {
    if (allConversations && allConversations.length > 0 && !selectedConversation) {
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      try {
        const res = await fetch(`/api/chat/get-messages?sessionId=${selectedConversation.id}`);
  ...
  const selectedOtherUser = selectedConversation ? getOtherUser(selectedConversation) : undefined

  // bugs -- fixed [this one need the s3 and dynamodb connection]
  const handleSendMessage = async () => {
    if ((!newMessage.trim() && !selectedAttachment) || !selectedConversation) return;
    ...
    const targetUserId = selectedConversation.participantIds.find(id => id !== currentUserId);
  ...
  const renderAttachment = (att: NonNullable<ChatMessage["attachments"]>[0]) => {
    if (!att) return null;
      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm">
        <div className="flex h-[850px] relative">
          ...
          <div className="flex-1 flex flex-col">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <Avatar className="w-10 h-10">
                    <AvatarImage src={selectedOtherUser?.avatar || "/placeholder.svg"} />
            ...
            {/* Messages */}
            <div className="flex-1 p-4 overflow-y-scroll custom-scrollbar" ref={chatContainerRef} >
