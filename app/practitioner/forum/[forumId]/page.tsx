...
export default function PractitionerForumDetailPage() {
  ...
  useEffect(() => {
    const fetchForumData = async () => {
      try {
        ...
        try {
          const forumsResponse = await fetch("/api/practitioner/forums");
        ...
        const forumResponse = await fetch(
          `/api/practitioner/forums/${forumId}`,
        );
        if (!forumResponse.ok) {
        ...
        const postsResponse = await fetch(
          `/api/practitioner/posts?forumId=${forumId}`,
        );
        if (!postsResponse.ok) {
  ...
  const handleForumSelect = (selectedForumId: string) => {
    router.push(`/practitioner/forum/${selectedForumId}`);
  };
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          ...
          <button
            onClick={() => router.push("/practitioner/forum")}
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="sm:hidden">
        <ForumSidebar
          ...
          onForumCreated={() => {
            const fetchForums = async () => {
              try {
                const response = await fetch("/api/practitioner/forums");
                if (response.ok) {
          ...
          apiEndpoint="/api/practitioner/forums"
          baseRoute="/practitioner/forum"
        />
        ...
        <div className="bg-background">
          <PostList posts={sortedPosts} baseRoute="/practitioner/forum" />
        </div>
      </div>
      ...
      {showCreatePost && (
        <CreatePostAuth
          ...
          apiEndpoint="/api/practitioner/posts"
        />
      )}
    </div>
  );
