...
export default function UserForumDetailPage() {
  ...
  useEffect(() => {
    const fetchForumData = async () => {
      try {
        ...
        try {
          const forumsResponse = await fetch("/api/user/forums");
        ...
        const forumResponse = await fetch(`/api/user/forums/${forumId}`);
        if (!forumResponse.ok) {
        ...
        const postsResponse = await fetch(`/api/user/posts?forumId=${forumId}`);
        if (!postsResponse.ok) {
  ...
  const handlePostCreated = () => {
    ...
    const fetchPosts = async () => {
      try {
        const response = await fetch(`/api/user/posts?forumId=${forumId}`);
        if (response.ok) {
  ...
  const handleForumSelect = (selectedForumId: string) => {
    router.push(`/user/forum/${selectedForumId}`);
  };
